...
source('code/simOutbreak.R')

# set random number seed
set.seed(1234)

# read in line list data for USA ... because mex kinda looks


# data from https://github.com/midas-network/COVID-19/tree/master/data/cases/global/line_listings_nihfogarty

#go to the website and manually search for the best one 

linelist = vroom::vroom('https://raw.githubusercontent.com/midas-network/COVID-19/master/data/cases/global/line_listings_nihfogarty/2020_03_14_1800EST_linelist_NIHFogarty.csv')
...
num.CF = c(
  nrow(subset(yesUS,traveler>0))-sum(subset(yesUS,traveler>0)$death>0, na.rm = T),
  sum(subset(yesUS,traveler>0)$death>0, na.rm = T))
...
ts = vroom::vroom("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Confirmed.csv")
ts <- rename(ts, country_region = `Country/Region`)
#ts.natl = matrix(0,length(unique(ts$Country.Region)),ncol(ts)-4)
ts.natl = matrix(0,length(unique(ts$country_region)),ncol(ts)-4)
for(ii in 1:ncol(ts.natl)){
  ts.natl[,ii] = aggregate(ts[,4+ii],by=list(ts$country_region),FUN=sum)[,2]
}
row.names(ts.natl) = aggregate(ts[,4+ii],by=list(ts$country_region),FUN=sum)[,1]
...
tsd = vroom::vroom("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Deaths.csv")
tsd <- rename(tsd, Country.Region = `Country/Region`)
...
# count up local cases by day in the MX
#cases.US.total = c(rep(0,21),ts.natl['US',])
cases.US.total = c(rep(0,21),ts.natl['US',])

yesUS$reporting.date <- yesUS$`reporting date`
cases.US.imported =
  table(
...
pdf('cumulative_infections.pdf',width=5,height=4)
hist(
...
pdf('eventual_deaths.pdf',width=5,height=4)
hist(
...
pdf('infections_daily.pdf',width=5,height=4)
local.mat = t(matrix(
...
pdf('symptomatic_daily.pdf',width=5,height=4)
cases.mat = t(matrix(
...
pdf('symptomatic_detected.pdf',width=5,height=4)
cases.mat = t(matrix(
  unlist(lapply(local, function(x) x$cases)),
  length(local[[1]]$cases),
  replicates))
...
pdf('deaths_daily.pdf',width=5,height=4)
death.mat = t(matrix(
